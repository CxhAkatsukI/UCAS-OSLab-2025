...
/* Global pipe table and protection lock */
pipe_t pipes[PIPE_NUM];
static spin_lock_t pipe_lock;

/**
 * init_pipes - Initialize the pipe mechanism.
 *
 * Initializes the global spinlock and resets all pipe structures
 * in the system.
 */
...
/**
 * do_pipe_open - Open or create a named pipe.
 * @name: Name of the pipe to open.
 *
 * Searches for an existing pipe with the given name. If found, returns
 * its index. If not, allocates a new pipe.
 *
 * Return: Pipe index on success, -1 on failure (table full).
 */
...
/**
 * do_pipe_give_pages - Send memory pages into a pipe.
 * @pipe_idx: Index of the pipe.
 * @src: Virtual address of the source buffer (must be page aligned).
 * @length: Length of data to send (must be page aligned).
 *
 * Unmaps physical pages from the current process and queues them in the pipe.
...
/**
 * do_pipe_take_pages - Receive memory pages from a pipe.
 * @pipe_idx: Index of the pipe.
 * @dst: Virtual address to map the received pages to.
 * @length: Length of data to receive.
 *
 * Dequeues physical pages from the pipe and maps them into the current process.
 * Blocks if the pipe is empty.
 */
