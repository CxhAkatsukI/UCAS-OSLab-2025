...
/**
 * search_command_table - Search for a command handler.
 * @buf: Command name string.
 *
 * Return: Index in cmd_table if found, -1 otherwise.
 */
 ...
 * @max_len: The maximum number of characters to read (size of the buffer).
 *
 * Return: 0 on success.
 */
...
/**
 * run_command_loop - Main interactive command shell loop.
 * 
 * Repeatedly prompts for input, parses commands, and executes handlers.
 */
...
/**
 * cmd_exec - Execute a task by name or ID.
 * @args: String containing task name or ID.
 */
...
/**
 * cmd_help - Display supported commands.
 * @args: Unused argument string.
 */
...
/**
 * tokenize_string - Split a string into tokens.
 * @input_str: The string to tokenize.
 * @tokens: Output array to store tokens.
...
/**
 * cmd_write_batch - Write task sequence to SD card batch file.
 * @args: Space-separated task names.
 */
...
/**
 * parse_batch_file - Parse task names from buffer.
 * @buffer: Input buffer with newline-separated names.
 * @tasks_array: Output array for parsed names.
 * @max_tasks: Max tasks to parse.
 *
 * Return: Number of tasks parsed, or -1 on error.
 */
...
/**
 * cmd_exec_batch - Execute tasks defined in batch file.
...
/**
 * kernel_batch_handler - Handle batch execution logic in kernel.
 * @in_batch_mode: Flag indicating active batch.
 * @batch_current_task_idx: Index of current task.
 * @batch_total_tasks: Total tasks in batch.
 * @batch_io_buffer_val: I/O buffer value to pass.
 */
...
/**
 * cmd_wrq - Write programs into the ready queue.
 * @args: Space-separated task names.
 *
 * Initializes PCBs for specified tasks and adds them to the ready queue.
 * Use '*' to load a default set of tasks.
 */
...
/**
 * cmd_twrq - Write programs into the ready queue with timer interrupt.
 * @args: Space-separated task names.
 *
 * Initializes PCBs for specified tasks and adds them to the ready queue.
 * Enables timer interrupt and global interrupt.
 */
...
/**
 * cmd_vexec - Execute a program with virtual memory.
 * @args: Arguments string including task name.
 *
 * Initializes PCBs and adds the task to the ready queue.
 * Enables virtual memory execution context.
 */
